...
EPSILON = 0.5
SENSITIVITY = 0.0002  # graus (≈22m de deslocamento médio - próximo do original)
...
def snap_to_nearest_road(G, lat: float, lon: float, lat_orig: float, lon_orig: float, max_distance: float = MAX_SNAP_DISTANCE, force: bool = FORCE_SNAP):
    ...
    Returns:
        Tupla (lat_snapped, lon_snapped, edge_geom, success, rejected_by_distance)
    """
    if G is None or not MAP_MATCHING_AVAILABLE:
        return lat, lon, None, False, False
    ...
    try:
        ...
        try:
            ...
            if 'geometry' in edge_data:
                ...
                # If no geometry, create it from the nodes
                u_geom = Point(G.nodes[u]['x'], G.nodes[u]['y'])
                v_geom = Point(G.nodes[v]['x'], G.nodes[v]['y'])
                edge_geom = LineString([u_geom, v_geom])
            
            # Project the noisy point onto the edge
            noisy_point = Point(lon, lat)
            projected_point = edge_geom.interpolate(edge_geom.project(noisy_point))
            ...
            edge_geom = None # Point is on a node, no specific edge
        ...
        if snap_distance > max_distance:
            if force:
                ...
                return lat_snapped, lon_snapped, edge_geom, True, True  # success mas rejected_by_distance
            else:
                # Rejeitar snap - usar apenas ruído
                return lat, lon, None, False, True
        
        return lat_snapped, lon_snapped, edge_geom, True, False  # success, not rejected
...
def calculate_distance_from_projected_points(G, trajectory_points: list, min_distance_m: float = 1.0) -> tuple:
    """
    Calcula a distância da trajetória para pontos JÁ PROJETADOS na rede.
    - Se pontos estão na mesma aresta, calcula a distância ao longo da geometria da aresta.
    - Se pontos estão em arestas diferentes, usa roteamento de nó a nó como fallback.
    """
    if not MAP_MATCHING_AVAILABLE or G is None:
        return 0.0, 0, 0, 0
    ...
    # Extrai as posições para o filtro
    positions = [p['pos'] for p in trajectory_points]
    # filter_close_points retorna (pontos_filtrados, indices_mantidos)
    _, kept_indices = filter_close_points(positions, min_distance_m=min_distance_m)
    
    if len(kept_indices) < 2:
        return 0.0, 0, 0, 0

    # Filtra a trajetória original (com geometrias) usando os índices
    filtered_trajectory = [trajectory_points[i] for i in kept_indices]
    ...
    for i in range(len(filtered_trajectory) - 1):
        p1_data = filtered_trajectory[i]
        p2_data = filtered_trajectory[i+1]

        lat1, lon1 = p1_data['pos']
        lat2, lon2 = p2_data['pos']
        geom1 = p1_data['geom']
        geom2 = p2_data['geom']

        # Tenta usar a geometria da aresta para cálculo preciso
        if geom1 is not None and geom1 == geom2:
            try:
                # Calcula a distância ao longo da linha da aresta
                dist_on_edge = abs(geom1.project(Point(lon2, lat2)) - geom1.project(Point(lon1, lat1)))
                total_distance += dist_on_edge / 1000.0  # metros para km
                success_count += 1
                continue
            except Exception:
                # Se `project` falhar, usa o fallback abaixo
                pass

        # Fallback para roteamento de nó a nó (arestas diferentes, nós, etc.)
        try:
            node1 = ox.nearest_nodes(G, lon1, lat1)
            node2 = ox.nearest_nodes(G, lon2, lat2)

            if node1 == node2:
                # Nós mais próximos são os mesmos, usar Haversine como fallback final
                dist = calculate_gps_distance(lat1, lon1, lat2, lon2)
                total_distance += dist
                fallback_count += 1
            else:
                # Roteamento entre os nós
                route_dist = calculate_route_distance(G, node1, node2)
                if route_dist is not None:
                    total_distance += route_dist
                    success_count += 1
                else:
                    # Rota não encontrada, usar Haversine
                    dist = calculate_gps_distance(lat1, lon1, lat2, lon2)
                    total_distance += dist
                    fallback_count += 1
        except Exception:
            # Se `nearest_nodes` ou qualquer outra coisa falhar
            dist = calculate_gps_distance(lat1, lon1, lat2, lon2)
            total_distance += dist
            fallback_count += 1

    return total_distance, success_count, fallback_count, len(filtered_trajectory)
    ...
    for vehicle_id, group in df.groupby('vehicle_id'):
        ...
        if ENABLE_MAP_MATCHING and MAP_MATCHING_AVAILABLE:
            ...
            if G_start is not None:
                start_lat_private, start_lon_private, _, start_snapped, _ = snap_to_nearest_road(
            ...
            if G_end is not None:
                end_lat_private, end_lon_private, _, end_snapped, _ = snap_to_nearest_road(
        ...
        for seg_idx, seg_row in group.iterrows():
            ...
            if ENABLE_MAP_MATCHING and MAP_MATCHING_AVAILABLE:
                ...
                if G_seg is not None:
                    ...
                    seg_lat_priv, seg_lon_priv, edge_geom, snap_success, snap_rejected = snap_to_nearest_road(
            ...
            trajectory_points_priv.append({'pos': [seg_lat_priv, seg_lon_priv], 'geom': edge_geom})
        ...
        if MAP_MATCHING_AVAILABLE:
            ...
            center_lat = np.mean(lats_orig)
            center_lon = np.mean(lons_orig)
            G = get_road_network(center_lat, center_lon, radius=SEARCH_RADIUS * 2)
            trajectory_distance_orig, orig_success, orig_fallback, orig_filtered = calculate_trajectory_distance_with_routing(
                trajectory_points_orig, G, min_distance_m=10.0
            )

            # Obter grafo SEPARADO para trajetória com ruído (pontos podem estar deslocados)
            lats_priv = [p['pos'][0] for p in trajectory_points_priv]
            lons_priv = [p['pos'][1] for p in trajectory_points_priv]
            center_lat_priv = np.mean(lats_priv)
            center_lon_priv = np.mean(lons_priv)
            G_priv = get_road_network(center_lat_priv, center_lon_priv, radius=SEARCH_RADIUS * 2)
            # Calcular distância com ruído usando a nova função para pontos projetados
            trajectory_distance_priv, priv_success, priv_fallback, priv_filtered = calculate_distance_from_projected_points(
                G_priv, trajectory_points_priv, min_distance_m=1.0
            )

            # Mostrar estatísticas de roteamento
            if orig_fallback > 0 or priv_fallback > 0 or orig_filtered != len(trajectory_points_orig) or priv_filtered != len(trajectory_points_priv):
                ...
                print(f"      Ruído:    {len(trajectory_points_priv)} pontos → {priv_filtered} filtrados → {priv_success} rotas OK, {priv_fallback} fallbacks")
        else:
        ...
        # DEBUG: Diagnóstico: quantos pontos únicos existem no trajeto com ruído (privacidade)
        if len(trajectory_points_priv) > 1:
            unique_points_priv = set(tuple(p['pos']) for p in trajectory_points_priv)
            print(f"  [DEBUG] {vin}: {len(trajectory_points_priv)} pontos privados, {len(unique_points_priv)} únicos")
            if len(unique_points_priv) == 1:
                print(f"  ⚠️  {vin}: Map matching colapsou pontos! TODOS os pontos privados foram para o mesmo local: {trajectory_points_priv[0]['pos']}")
            elif len(unique_points_priv) < len(trajectory_points_priv) // 2:
                print(f"  ⚠️  {vin}: Muitos pontos privados colapsaram ({len(unique_points_priv)}/{len(trajectory_points_priv)})")
        ...
        trajectories.append({
            ...
            'trajectory_private': [p['pos'] for p in trajectory_points_priv],
...
def main():
    ...
    # Montar nome da pasta de saída com epsilon e sensibilidade
    eps_str = f"e{EPSILON:g}".replace('.', '_')
    sens_str = f"s{SENSITIVITY:.6f}".rstrip('0').rstrip('.')
    sens_str = sens_str.replace('.', '_')
    out_dir_name = f"e{EPSILON:g}-s{SENSITIVITY:.6f}".replace('.', '_').replace('s0_', 's0.')
    ...
    if len(sys.argv) > 2:
        ...
        output_file = os.path.join(out_dir, f"{base_name}_processed.csv")
    ...
    # Executar visualize_trips.py automaticamente no JSON gerado
    try:
        import subprocess
        vis_script = os.path.join(script_dir, 'visualize_trips.py')
        if os.path.exists(vis_script):
            print(f"\n🚦 Executando visualização automática: {vis_script} {json_file}\n")
            subprocess.run([sys.executable, vis_script, json_file], check=True)
        else:
            print(f"⚠️  Script visualize_trips.py não encontrado em {vis_script}. Pulei visualização automática.")
    except Exception as e:
        print(f"⚠️  Erro ao executar visualização automática: {e}")
