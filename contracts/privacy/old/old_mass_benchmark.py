...
def print_tx_timing(row: Dict[str, Any]) -> None:
    scenario = row.get("scenario")
    csv_file = row.get("csv_file")
    repeat_index = row.get("repeat_index")
    tx_method = row.get("tx_method")
    tx_status = row.get("tx_status")
    tx_seconds = row.get("tx_seconds")
    error = row.get("error")

    if tx_method == "oracle_cancelled":
        print(
            f"[bench] scenario={scenario} csv={csv_file} rep={repeat_index} no_tx=offset_zero",
            flush=True,
        )
        return

    seconds_str = f"{tx_seconds:.6f}" if isinstance(tx_seconds, (int, float)) else "n/a"
    status_str = str(tx_status) if tx_status is not None else "n/a"
    method_str = str(tx_method) if tx_method is not None else "n/a"

    msg = (
        f"[bench] scenario={scenario} csv={csv_file} rep={repeat_index} "
        f"method={method_str} status={status_str} tx_seconds={seconds_str}"
    )
    if error:
