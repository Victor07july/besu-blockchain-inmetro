#!/usr/bin/env python3
"""
Envia ETH para o contrato CarbonCreditNFT_E1 (saldo usado no resgate ZK).
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Envia ETH para o contrato CarbonCreditNFT_E1")
    parser.add_argument("--rpc-url", default=DEFAULT_RPC_URL, help="URL RPC do no")
    parser.add_argument("--private-key", default=DEFAULT_PRIVATE_KEY, help="Chave privada do pagador")
    parser.add_argument("--deployment-file", default=str(default_deploy), help="deployment_info.json")
    parser.add_argument("--amount-wei", type=int, default=None, help="Valor em wei")
    parser.add_argument("--amount-eth", type=float, default=None, help="Valor em ETH")
...
def main() -> None:
    ...
    if args.amount_wei is None and args.amount_eth is None:
        raise ValueError("Informe --amount-wei ou --amount-eth")
    ...
    if args.amount_wei is not None:
        value = int(args.amount_wei)
    else:
        value = int(w3.to_wei(args.amount_eth, "ether"))
    ...
    print("[ok] Fund tx:", receipt.transactionHash.hex())
